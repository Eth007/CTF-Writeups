# Librarian (175 pts) - Pwn

This is mostly a standard ret2libc challenge, with a bit of a twist. The challenge first asks for a password generated by a random number generator, which can be bypassed because the RNG is unseeded, creating the same number each time. The rest of the challenge can be seen from the below solve script:
